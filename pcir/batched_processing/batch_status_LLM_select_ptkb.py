...
from pcir.utils import ensure_unique_path
...
def process_content(content):
    """
    Processes the content to extract 'Provenance'.
    """
    try:
        match_prov = re.search(r'Provenance:\s*(.*)', content, re.DOTALL)
...
def load_batch_id_mapping(folder):
    if "2024" in folder:
        ...
        mapping_file = os.path.join(folder, "batch_id_mapping_LLM_select_ptkb.json")
        ...
        with open(processed_output_path, "w", encoding="utf-8") as outfile:
            for line in result_file.text.strip().split('\n'):
                try:
                    ...
                    provenance = process_content(content)
...
def main():
    parser = argparse.ArgumentParser(description="Check batch status and download & process results for select_ptkb_Xshot.")
