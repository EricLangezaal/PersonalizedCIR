...
def append_batch_id_mapping(folder, filename, batch_id):
    if "2024" in folder:
        mapping_file = os.path.join(folder, "batch_id_mapping_LLM_select_ptkb_2024.json")
    else:
        mapping_file = os.path.join(folder, "batch_id_mapping_LLM_select_ptkb_2023.json")
...
def prepare_batch_file(input_path, batch_file_path, model_engine, shot, random_examples, seed):
    ...
    with open(input_path) as infile:
        for line in infile:
            ...
            prompt = (
                "For an information-seeking dialog, please select the user information that will help answer this question (if there is any).\n"
            )
            if d:
            ...
            batch_requests.append({
                ...
                "body": {
                    "model": model_engine,
                    "messages": [
                        {"role": "system", "content": "You are a good information selector"},
...
def upload_and_submit_batch(batch_file_path, folder):
    ...
    batch_response = client.batches.create(
        ...
        metadata={"description": "Batch processing for select_ptkb_Xshot"},
    )
...
def main():
    parser = argparse.ArgumentParser(description="Prepare and submit a batched API job for select_ptkb_Xshot.")
    ...
    if args.batch_file_path is None:
        file_name = f"batch_LLM_select_ptkb_{args.shot}shot.jsonl"
